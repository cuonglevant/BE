...
def process_p1_answers(image_path=None, show_images=False, save_images=False):
    ...
    Args:
        image_path (str): Đường dẫn đến file ảnh. Nếu None sẽ dùng "p12.jpg" mặc định
    ...
    """
    # Luôn tắt hiển thị GUI để tránh lỗi môi trường server
    show_images = False  # Không dùng GUI, chỉ xử lý logic và lưu ảnh nếu cần
    ...
    # Vẽ tất cả contours để debug (nếu cần lưu ảnh)
    ...
    for i, contour in enumerate(contours):
        ...
        if len(approx) == 4:
            ...
            color = (0, 255, 0) if 200000 < area < 250000 else (0, 255, 255)
            thickness = 3 if 200000 < area < 250000 else 2
            cv2.drawContours(all_contours_image, [contour], -1, color, thickness)
            M = cv2.moments(contour)
            if M["m00"] != 0:
                cx = int(M["m10"] / M["m00"])
                cy = int(M["m01"] / M["m00"])
                cv2.putText(all_contours_image, f"{i}: {int(area)}", (cx-30, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    ...
    for idx, (contour, approx, area, original_idx) in enumerate(qualified_contours):
        ...
        try:
            # Vẽ contour được chọn (nếu cần lưu ảnh)
            ...
            for i, point in enumerate(approx.reshape(4, 2)):
                ...
                cv2.putText(selected_contour_image, f"P{i+1}", (point[0]+10, point[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            if save_images:
            ...
            # Nhận dạng ô đã tô
            ans = []
            numbered_image = cropped_paper.copy() if save_images else None
...
def main():
    """Hàm main để test"""
    # Test nhận diện đáp án, không dùng GUI
